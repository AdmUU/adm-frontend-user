    ...
    <div class="popup">
      ...
      <template v-else>
        <div class="image-container">
          <img
            v-if="imageSrc"
            :src="imageSrc"
            :style="{ transform: `rotate(${imageTransform}deg)` }"
          />
      ...
      <template v-else>
        <div class="msgbox">
          <div v-show="isError" class="error">{{
            $t('plugin.ljk123-captcha.base.fail')
          }}</div>
          <div v-show="isSuccess" class="success">{{
            $t('plugin.ljk123-captcha.base.success', successOption)
          }}</div>
        </div>
      </template>
      <div
      ...
      >
        <div class="slider-bg" :style="'width:' + draggingWidth + 'px'">
          <div
            class="slider-container-text"
            :style="
              slider?.parentElement
                ? 'width:' + slider.parentElement.offsetWidth + 'px'
                : ''
            "
            >{{ $t('plugin.ljk123-captcha.angle.sliderTxt') }}</div
          >
        </div>
        <div ref="slider" class="slider" :style="positionLeftStyle">
          <div class="line"></div>
        </div>
        <div class="slider-container-text slidetounlock">{{
          $t('plugin.ljk123-captcha.angle.sliderTxt')
        }}</div>
      </div>
      <div class="helper">
        <span @click="callGetCaptcha">{{
          $t('plugin.ljk123-captcha.base.refresh')
        }}</span>
        <span v-if="props.feedbackUrl" @click="onFeedback">{{
          $t('plugin.ljk123-captcha.base.faceback')
        }}</span>
      </div>
    </div>
    <div class="mask"></div>
...
<script setup>
  ...
  const sliderWidth = computed(() => {
    return (
      (slider.value?.parentElement.offsetWidth || 1) -
      (slider.value?.offsetWidth || 0)
    );
  });
  const imageTransform = computed(() => {
    return Math.max(
      0,
      Math.min((currentX.value / sliderWidth.value) * maxAngle, maxAngle)
    );
  });
