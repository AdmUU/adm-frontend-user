...
<script lang="tsx" setup>
  ...
  const pingType = ref('single');
  const addressType = ref('');
  const dictCode = ref('');
  const dictValue = ref('');
  const taskId = ref('');
  const pingFormKey = ref(0);
  ...
  function updatePingtype(
    type: string,
    nodeType: string,
    pingDictCode: string,
    pingDictValue: string,
    pingTaskId: string
  ) {
    pingType.value = type;
    addressType.value = nodeType;
    dictCode.value = pingDictCode;
    dictValue.value = pingDictValue;
    taskId.value = pingTaskId;
  }
